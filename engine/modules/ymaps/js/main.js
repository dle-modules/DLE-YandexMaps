...
doc
	...
	.on('submit', '[data-ladda-submit]', function (e) {
		...
		var $this        = $(this),
		    progress     = 0,
		    laddaLoadNew = $this.find('.ladda-button').ladda();
	...
	.on('click', '[data-point-change]', function () {
		...
		$.magnificPopup.open({
			...
			items: {
				src: 'engine/ajax/ymaps/editmarker.php'
			},
			ajax: {
				settings: {
					data: {
...
// pre-submit callback
function processStartNew(formData, jqForm, options) {
...
function processDoneNew(responseText, statusText, xhr, $form) {
	...
	var formContent    = $form.html(),
	    responseResult = (formContent) ? formContent : responseText;
...
function init() {
	...
	doc
		.on('mapInit', function () {
			var controls = [],
			    height   = ($('#mapHeight').val()) ? $('#mapHeight').val() : 400,
			    $map     = $('#map');
...
function showSelectedPoint(obj) {
	var $this             = obj,
	    selectedData      = $this.find('option:selected').data(),
	    $marker           = $('.selected-marker'),
	    $iconLayout       = $('.iconLayout'),
	    $iconImageSize0   = $('.iconImageSize0'),
	    $iconImageSize1   = $('.iconImageSize1'),
	    $iconImageOffset0 = $('.iconImageOffset0'),
	    $iconImageOffset1 = $('.iconImageOffset1');

	$marker.prop('src', selectedData.selectImg);
	$('.set-icon-color-input').remove();
	$('.selected-marker').css('background', 'none');
