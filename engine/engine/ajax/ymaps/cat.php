...
$yMapCfg = [// 'template' => !empty($template) ? $template : $cfg['moduleName'] . '/all/default',
	...
	'catId'       => !empty($_REQUEST['catId']) ? $_REQUEST['catId'] : (!empty($arConf['catId']) ? $arConf['catId'] : ''), // Категории для показа (id одной категории или this)
...
if (!$yMap) {
	...
	if ($yMapCfg['catId'] || $yMapCfg['notCatId']) {
		...
		$catArr = ($yMapCfg['notCatId']) ? getDiapazone($yMapCfg['notCatId'], $yMapCfg['notSubcats']) : ($yMapCfg['subcats']) ? get_sub_cats($yMapCfg['catId']) : $yMapCfg['catId'];
